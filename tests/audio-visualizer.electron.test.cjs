...
test('visualizer updates independently with six styles, a picker, saved choices and released previews', { timeout: 120000 }, async() => {
  ...
  try {
    ...
    const oldDirectory = path.join(catalogRoot, 'audio-visualizer/1.0.0')
    ...
    const oldCatalog = { schemaVersion: 1, plugins: [{ id: 'audio-visualizer', version: '1.0.0', apiVersion: 1, path: `audio-visualizer/1.0.0/${oldFile}`, bytes: archive.length, sha256: createHash('sha256').update(archive).digest('hex') }] }
    ...
    assert.equal(new Set(previews).size, 6, 'Each style has a distinct preview')
    ...
    await page.locator('[data-plugin-visualizer="main"][data-visualizer-style="bars"]').waitFor()
    await choose(dialog, 'bars')
    await page.locator('[data-plugin-visualizer="main"][data-visualizer-style="bars"]').waitFor()
    ...
    await choose(dialog, 'ring')
    assert.equal(await page.locator('[data-plugin-visualizer="main"]').getAttribute('data-visualizer-style'), 'bars')
    ...
    await desktop.locator('[data-plugin-visualizer="desktop"][data-visualizer-style="ring"]').waitFor({ timeout: 20000 })
    ...
    dialog = await openPicker(page)
    await hasPixels(page, '[data-visualizer-dialog] [data-visualizer-preview="bars"]')
    ...
    assert.deepEqual(JSON.parse(await fs.readFile(preferencesFile, 'utf8')), { version: 1, main: 'bars', desktop: 'ring' })
    ...
    await showPlayer(page)
    dialog = await openPicker(page)
    assert.equal(await dialog.locator('[data-visualizer-option="bars"] input').isChecked(), true)
    await dialog.locator('[data-visualizer-surface="desktop"]').click()
    assert.equal(await dialog.locator('[data-visualizer-option="ring"] input').isChecked(), true)
    await page.evaluate(() => window.lxData.updateSetting({ 'common.langId': 'en-us' }))
    await dialog.getByRole('radio', { name: 'Pulse ring', exact: true }).waitFor()
