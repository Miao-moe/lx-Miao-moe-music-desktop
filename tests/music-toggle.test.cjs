...
function modalFixture(searchMusic, current = original) {
  ...
  const imports = {
    ...
    '@renderer/utils/musicSdk': { searchMusic },
  ...
  const modal = { ...options.data(), musicInfo: original, show: true, $emit: (...args) => emitted.push(args), $t: key => key }
...
test('loading uses the ranked candidate groups and previews the clicked item directly', async() => {
  const { modal, played } = modalFixture(async() => results)
  modal.loadList()
  await flush()
...
test('the first preview starts exactly once when playback was idle', async() => {
  const { modal, played } = modalFixture(async() => results, null)
  modal.loadList()
  await flush()
  modal.handlePlay(match)
  assert.deepEqual(played, [match.id])
...
test('confirmation rejects a selection from a different tab or a closed dialog', async() => {
  const { modal, emitted } = modalFixture(async() => results)
  modal.loadList()
  await flush()
  modal.handlePlay(match)
  modal.source = 'kw'
...
test('late results cannot reopen a closed search or replace a newer song search', async() => {
  let finish
  const { modal } = modalFixture(async() => new Promise(resolve => { finish = resolve }))
  modal.loadList()
  modal.handleClose()
  finish(results)
  await flush()
  ...
  modal.loadList()
  finish([{ source: 'wy', list: [{ ...unrelated, id: 'wy_alternative', source: 'wy' }] }])
  await flush()
  oldFinish(results)
  await flush()
  assert.equal(modal.list[0].id, 'wy_alternative')
})
