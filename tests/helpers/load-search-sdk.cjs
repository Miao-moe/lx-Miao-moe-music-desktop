...
module.exports = function loadSearchSdk(respond) {
  ...
  const overrides = new Map(Object.entries({
    ...
    'index.js': { formatPlayTime: String, sizeFormate: String, decodeName: value => String(value ?? '') },
    ...
    'musicSdk/wy/utils/crypto.js': { weapi: data => data },
  ...
  return { load: name => load(path.join(root, name)), sdk: source => load(path.join(root, 'musicSdk', source, 'musicSearch.js')).default, calls, logs, waits, messages }
}
