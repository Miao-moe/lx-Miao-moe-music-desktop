...
async function launch({ record = false, profilePath, initializeMotion = true, reducedMotion = 'no-preference', rendererPath = process.env.LX_TEST_RENDERER_PATH } = {}) {
...
require(${JSON.stringify(path.join(project, 'dist/main.js'))});`)
  ...
  await page.waitForFunction(() => {
    ...
    const enabled = setting['common.isShowAnimation'] && setting['ui.smoothAnimation'] &&
      !(setting['ui.followSystemMotion'] === true && matchMedia('(prefers-reduced-motion: reduce)').matches)
