...
export default {
  ...
  async getListDetail2(id, tryNum = 0) {
    ...
    if (body.req_1?.code !== this.successCode || !Array.isArray(body.req_1.data?.songlist) || !body.req_1.data.dirinfo) throw new Error('failed')
  ...
  async getListDetail(id, tryNum = 0) {
    ...
    if (body.subcode !== this.successCode || !body.cdlist?.length) return this.getListDetail2(id)
