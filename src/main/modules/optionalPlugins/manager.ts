...
import { isPluginId, OFFICIAL_PLUGIN_ROOT, PLUGIN_API_VERSION, type PluginCatalog, type PluginCatalogEntry, type PluginId, type PluginManifest, type PluginStoreSnapshot } from '@common/optionalPlugins'

const MAX_PACKAGE_BYTES = 20 * 1024 * 1024
const MAX_UNPACKED_BYTES = 40 * 1024 * 1024
const digest = (data: Buffer) => createHash('sha256').update(data).digest('hex')
const validVersion = (value: unknown): value is string => typeof value == 'string' && /^\d+\.\d+\.\d+$/.test(value)
...
export const parseCatalog = (bytes: Buffer): PluginCatalog => {
  ...
  if (catalog.schemaVersion !== 1 || !Array.isArray(catalog.plugins) || catalog.plugins.length > 20) throw new Error('Invalid plugin catalog')
  const ids = new Set<string>()
  for (const plugin of catalog.plugins) {
    if (!isPluginId(plugin.id) || ids.has(plugin.id) || !validVersion(plugin.version) || !Number.isSafeInteger(plugin.apiVersion) ||
...
const validateManifest = (manifest: PluginManifest, id: PluginId) => {
  if (!manifest || manifest.id !== id || !validVersion(manifest.version) || manifest.apiVersion !== PLUGIN_API_VERSION ||
...
export const unpackPlugin = (bytes: Buffer, entry: PluginCatalogEntry) => {
  ...
  if (archive.manifest.version !== entry.version || !archive.files || typeof archive.files != 'object' || Object.keys(archive.files).length !== archive.manifest.files.length) throw new Error('Plugin package does not match the catalog')
...
export class PluginManager {
  ...
  private async readRegistry(): Promise<Registry> {
    await this.prepare()
    try {
      return JSON.parse(await fs.readFile(this.child('installed.json'), 'utf8')) as Registry
    } catch (error: any) {
  ...
  private async removeDirectory(name: string) {
    const target = this.child(name)
    if (!/^(sound-effects|audio-visualizer|install)-[a-f0-9-]{36}$/.test(name)) throw new Error('Invalid installed plugin directory')
    await this.prepare()
  ...
  async snapshot(): Promise<PluginStoreSnapshot> {
    ...
    for (const [id, record] of Object.entries(registry)) {
      ...
      try {
        const directory = this.child(record.directory)
  ...
  async refresh() {
    try {
      this.catalog = parseCatalog(await this.fetchBinary(OFFICIAL_PLUGIN_ROOT + 'catalog.json', 128 * 1024)).plugins
  ...
  async install(id: PluginId) {
    return this.exclusive(async() => {
      ...
      if (entry.apiVersion !== PLUGIN_API_VERSION) throw new Error('Plugin requires a different application version')
      ...
      try {
        ...
        this.revision++
        committed = true
        if (previous) await this.removeDirectory(previous.directory).catch(console.error)
      } finally {
        await this.removeDirectory(temporaryName)
        if (!committed) await this.removeDirectory(directoryName)
      }
      return this.snapshot()
    })
  }
  ...
  async uninstall(id: PluginId) {
    return this.exclusive(async() => {
      ...
      if (previous) {
        ...
        await this.removeDirectory(previous.directory)
